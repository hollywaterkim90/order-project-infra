...
module "eks" {
  ...
  # 🔒오직 Bastion Host에서만 EKS Control Plane(API Server)에 접근 가능하도록 격리합니다.
  cluster_security_group_additional_rules = {
    ingress_bastion = {
      description              = "Allow EKS API access only from Bastion Host"
      protocol                 = "tcp"
      from_port                = 443 # EKS API 서버와 통신하는 기본 보안 포트
      to_port                  = 443
      type                     = "ingress"
      # bastion.tf에서 생성될 bastion_sg의 ID를 동적으로 추적하여 방화벽을 뚫어줍니다.
